...
export function getItemState(asin: string) {
    return Execution.transaction("Amazon", getCurrentFilename())
        .then(val =>
            WebExecution.get({
                url: `https://www.amazon.co.jp/gp/product/${asin}?language=ja_JP`
            }, parse)
        )
        .then(val => {
            if (val.deleted) {
                return Execution.transaction()
                    .then(_ => DBExecution.amazon(rep => rep.deleteItem(asin)))
                    .then(_ => Execution.throw())
            } else {
...
// https://www.amazon.co.jp/gp/product/B095CQL96V
// https://www.amazon.co.jp/gp/product/B076BN6DQG
// https://www.amazon.co.jp/gp/product/B095CQL96V
    ...
    return Execution.transaction("Amazon", getCurrentFilename())
        .then(val =>
            WebExecution.get({
                ...
                params: { api_key: "68d6de532946616aae283bc9fd0ea7a2", keep_headers: true, url: `https://www.amazon.co.jp/gp/product/${asin}?language=ja_JP` },
                headers: { 
