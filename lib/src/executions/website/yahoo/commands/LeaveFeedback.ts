...
export function leaveFeedback(
    ...
    return WebExecution.webTransaction(session, "YahooDriver", getCurrentFilename())
        .setCookie(val => val)
        .thenGet("GetFormData", 
            val => ({
                url: `https://auctions.yahoo.co.jp/jp/show/leavefeedback?aID=${aid}&t=${targetId}`
            }),
            (doc, val) => ({
                form: getFormHiddenInputData(doc, "//form"),
                rating: rating,
