...
export function setupAccount(session: YahooSession, setting: YahooAccountSetting, password: string) {
    return Execution.transaction(arguments, "Yahoo", getCurrentFilename())
        ...
        .then(val => {
            return DBExecution.yahoo(rep => {
                return rep.setAccountDesiredSetting(session.account.username, setting.id);
            })
        });
}