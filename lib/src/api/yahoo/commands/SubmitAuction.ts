...
export const submitAuction = createWebTransaction<{ 
    session: Cookie,
    auction: AuctionSubmission,
    ...
    .thenPost("Preview",
        val => {
            val.images.forEach((img, i) => {
                val.auction[`ImageFullPath${i+1}`] = img.url;
                val.auction[`ImageWidth${i+1}`] = img.width;
                val.auction[`ImageHeight${i+1}`] = img.height;
            });
            return {
    ...
    .thenPost("Submit",
        val => ({
            ...
            form: Object.assign(val.auction, val.form)
        }),
        (doc, val) => toNotNull({
